...
*Setup instructions for a Gitian build of Ulord Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Ulord
...
- [Building Ulord Core](#building-ulord-core)
...
Clone the git repositories for Ulord Core and Gitian.
...
in the Ulord Core repository under 'Fetch and build inputs' to install sources which require
...
To build Ulord Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Ulord Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/ulord/.git/
...
and the ulord git repository with the desired tag must both be available locally, and then gbuild must be
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/ulord-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/ulord-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/ulord-osx-build.assert
```
