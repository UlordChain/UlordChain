...
UniValue nametrie(const UniValue& params, bool fHelp)
{
    ...
    std::vector<unsigned char>vchName(sName.begin(),sName.end());
    std::vector<unsigned char>vchAddr(addr.begin(),addr.end());
    CAmount nAmount = AmountFromValue(params[2]);
    CWalletTx wtx;
...
UniValue updatenametrie( const UniValue & params, bool fHelp)
{
	 if(!EnsureWalletIsAvailable(fHelp))
        return NullUniValue;
    if ( fHelp || params.size() != 3 )
    ...
    std::vector<unsigned char>vchAddr(addr.begin(),addr.end());
    CAmount nAmount = AmountFromValue(params[2]);
    isminefilter filter = ISMINE_NAME;
    UniValue entry;
    if ( !pwalletMain->mapWallet.count(hash) )
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY,"Invalid or non-wallet transaction id");
    const CWalletTx &wtx = pwalletMain->mapWallet[hash];
    ...
    {   
        ...
        {   
            ...
            {   
                ...
                UpdateNameTrie(vchName, claimId, vchAddr, nAmount, wtxNew, wtx, i);
                fFound = true;
            }           
        }
    }
...
void CreateNameTrie(CScript& claimScript, CAmount nAmount, CWalletTx& wtxNew)
{
	//check amout
	if ( nAmount <= 0 )
	{
    ...
    {   
        ...
        {   
            ...
            {   
                ...
                else if ( ((op == OP_NAME_TRIE ) && (vvchParams.size() != 2)) || ((op == OP_NAME_UPDATE) && (vvchParams.size() != 3)) )
                {   
                    LogPrintf("%s(): Wrong number of params to name claim script. Got %d ,expected %d. Txid: %s",__func__,vvchParams.size(),((op == OP_CLAIM_NAME) ? 2:3),wtx.GetHash().ToString());
                    continue;
                }
                ...
                if ( op == OP_NAME_TRIE )
                {
                    uint160 claimId = ClaimIdHash(wtx.GetHash(),s.vout);
                    entry.push_back(Pair("claimId",claimId.GetHex()));
                    string addr(vvchParams[1].begin(),vvchParams[1].end());
                    entry.push_back(Pair("Ulordaddress",addr));
                }
                else if( op == OP_NAME_UPDATE)
                {
                    uint160 claimId(vvchParams[1]);
                    entry.push_back(Pair("claimId",claimId.GetHex()));
                    string addr(vvchParams[2].begin(),vvchParams[2].end());
                    entry.push_back(Pair("Ulordaddress",addr));
                }
                entry.push_back(Pair("txid",wtx.GetHash().ToString())); 
                entry.push_back(Pair("account",strSentAccount));
...
UniValue listnametrie(const UniValue &params, bool fHelp)
{
    ...
    if ( fHelp || params.size() > 3 )
        throw runtime_error(
        "listnametrie all acountname\n"
        "Return a list of all transactions about accountnames.\n"
        "\nArguments\n"
        ...
        "[\n"
        "  {\n"
        "    \"name\":\"accontname\",        (string) The name that is nametrie.\n"
        "    \"claimId\":\"claimId\",         (string) The claimId of the claim.\n"
        "    \"Ulordaddress\":\"Ulordaddress\"(string, required) The ulord address for bind accountname.\n"
        "    \"account\":\"accountname\",     (string) The account name associated with the transaction. \n"
        "                                              It will be \"\" for the default account.\n"
        "    \"address\":\"ulordaddress\",  (string) The ulord address of the transaction.\n"
        "    \"category\":\"name\"            (string) Always name\n"
